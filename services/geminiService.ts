        ...
        parts: [
          ...
          },
          {
            text: `Analizza questa etichetta di vino e estrai i dettagli in formato JSON. 
                   Cerca di identificare: Nome, Produttore, Annata (Vintage), Tipo (Rosso, Bianco, Rosato, Bollicine, Dolce/Passito), Vitigno (Grape), Regione, e suggerisci un abbinamento cibo (Pairing).
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      contents: `L'utente vuole inserire un vino chiamato: "${query}". 
                 ...
                 Campi: name, producer, vintage (se non specificato usa "NV"), type (Rosso, Bianco, Rosato, Bollicine, Dolce/Passito), grape, region, pairing.`,
      config: {
        responseMimeType: "application/json",
        responseSchema: {
          type: Type.OBJECT,
